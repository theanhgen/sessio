...
- **`↵` resume** — runs `claude --resume <id>` in that session's original working directory. If the session is **already running** (`◉`), sessio stops rather than pointing a second `claude` at the same transcript: it raises that window, and when the session is a **split** the window isn't currently showing, it walks the splits to focus the right one — a window reports the title of whichever split has focus, so each step says where it landed. Either way the duplicate takes an explicit second `↵`. A session in a background **tab** still can't be reached: tabs aren't exposed to the outside world at all, so there sessio names the pid and tty and leaves the finding to you. Under **Ghostty**, it opens the session in a **new window** (`ghostty +new-window`) and leaves sessio running as a launcher, so you can fire off several sessions; in any other terminal it hands over the current window as before. Press **`^o`** instead to resume in **this** window (replacing sessio) even under Ghostty — the escape hatch when you don't want a new window.
